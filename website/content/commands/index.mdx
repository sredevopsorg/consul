...
## Accepts URL-Invalid Characters

Many CLI commands use Consul HTTP API endpoints that require resource names in the request URL.
To enable the CLI to interact with resources containing characters that are invalid in a URL
path segment, the CLI will URL-encode parameters to be included in the HTTP API path,
and then the [HTTP API endpoint will URL-decode those parameters](/api-docs#accepts-url-encoded-resource-names).

Though any HTTP API resource name *can* be used with the CLI
([with a few exceptions](/api-docs#exceptions-characters-that-are-always-invalid)),
